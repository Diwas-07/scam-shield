...
const emergingThreats = [
  {
    ...
    trend: 'rising',
    change: '+340%',
    desc: 'Scammers clone the voices of family members using AI to request emergency funds via phone.',
    severity: 'critical',
  },
  {
    title: 'Deepfake Investment Ads',
    trend: 'rising',
    change: '+210%',
    desc: 'Fraudulent investment schemes use deepfake videos of celebrities to appear legitimate.',
    severity: 'high',
  },
  {
    ...
    trend: 'rising',
    change: '+180%',
    desc: 'Fake SMS/WhatsApp notifications about held parcels lead to credential harvesting sites.',
    severity: 'high',
  },
  {
    ...
    trend: 'stable',
    change: '+5%',
    desc: 'Long-term romance-investment hybrid scams where trust is built before financial exploitation.',
    severity: 'high',
  },
  {
    title: 'Lottery/Prize Notifications',
    trend: 'falling',
    change: '-12%',
    desc: 'Traditional prize scams declining as public awareness improves.',
    severity: 'medium',
  },
]
    ...
    <div className="p-8 min-h-screen grid-bg">
            ...
            {(['7d', '30d', '90d'] as const).map(p => (
              <button
                ...
                className={`px-4 py-2 text-xs font-mono transition-all ${
                  selectedPeriod === p ? 'bg-acid text-ink' : 'text-muted hover:text-frost'
                }`}
              >
                {p}
              </button>
      ...
      <section className="mb-6">
        ...
        <div className="space-y-3">
          {emergingThreats.map((threat) => (
            <div key={threat.title} className="card-dark rounded-xl p-4 flex items-start gap-4 group hover:-translate-y-0.5 transition-transform">
              <div className="flex items-center gap-2 mt-1">
                {threat.trend === 'rising' ? (
                  <TrendingUp size={16} className="text-signal" />
                ) : threat.trend === 'falling' ? (
                  <TrendingDown size={16} className="text-safe" />
                ) : (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`text-xs font-mono px-2 py-0.5 rounded-full ${
                    threat.severity === 'critical' ? 'badge-high' : 
                    threat.severity === 'high' ? 'badge-high' : 'badge-medium'
                  }`}>
              ...
              <div className={`text-sm font-mono font-bold ml-4 flex-shrink-0 ${
                threat.trend === 'rising' ? 'text-signal' : 
                threat.trend === 'falling' ? 'text-safe' : 'text-warn'
              }`}>
                {threat.change}
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="card-dark rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={MOCK_STATS.monthlyTrend}>
              <defs>
      ...
      <div className="card-dark rounded-2xl overflow-hidden">
        ...
        <table className="w-full table-dark">
          ...
          <tbody>
            {MOCK_STATS.scamTypeBreakdown.map((item, i) => {
              const trends = ['+23%', '+14%', '+8%', '+31%', '+6%', '+45%', '+2%', '-3%']
              const trendVal = trends[i]
